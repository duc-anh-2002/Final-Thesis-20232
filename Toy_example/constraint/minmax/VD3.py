...
def constraint1(X):
    ...
    # print(-1*(np.dot(grad_f1[0].detach().numpy(), X[1:]) + y1.detach().numpy() - X[0]))
...
def constraint_g1(X):
    g1 = g(x, 1)
    grad_g1 = torch.autograd.grad(g1, x, create_graph=True)
    return -1*(np.dot(grad_g1[0].detach().numpy(), X[1:-1]) + g1.detach().numpy() - X[-1])

def constraint_g2(X):
    g2 = g(x, 2)
    grad_g2 = torch.autograd.grad(g2, x, create_graph=True)
    return -1*(np.dot(grad_g2[0].detach().numpy(), X[1:-1]) + g2.detach().numpy() - X[-1])

def constraint_g3(X):
    g3 = g(x, 3)
    grad_g3 = torch.autograd.grad(g3, x, create_graph=True)
    return -1*(np.dot(grad_g3[0].detach().numpy(), X[1:-1]) + g3.detach().numpy() - X[-1])

def constraint_g4(X):
    g4 = g(x, 4)
    grad_g4 = torch.autograd.grad(g4, x, create_graph=True)
    return -1*(np.dot(grad_g4[0].detach().numpy(), X[1:-1]) + g4.detach().numpy() - X[-1])

def constraint_g5(X):
    g5 = g(x, 5)
    grad_g5 = torch.autograd.grad(g5, x, create_graph=True)
    return -1*(np.dot(grad_g5[0].detach().numpy(), X[1:-1]) + g5.detach().numpy() - X[-1])
...
def project_x(x, g):
    ...
    def constraints(y):
        return np.array([g(y, 1), g(y, 2), g(y, 3), g(y, 4), g(y, 5)])
...
def solve_p(X, x): 
    ...
    # if 4 in l_delta:
    #     constraint.append({'type':'eq', 'fun': constraint_g4})
    # if 5 in l_delta:
    #     constraint.append({'type':'eq', 'fun': constraint_g5})
    res = minimize(objective, x0= X, args = (x), method='SLSQP', constraints=constraint, tol=1e-6)
    return res.x 


def gradient(X, x, kappa, sigma, eta, iter):
    x_new = x.detach().numpy() + kappa * np.array(X[1:-1])
    x_new = project_x(x_new, g)
    # print(x_new)
    if (F(x_new) <= F(x.detach().numpy()) - kappa * eps * np.linalg.norm(X[1:-1])**2):
...
def gradient_1(X, x):
    kappa = 1
    while (F(x.detach().numpy() + kappa * np.array(X[1:])) > F(x.detach().numpy()) - kappa * eps * np.linalg.norm(X[1:])**2):
        kappa = kappa/2
    # print(kappa)
...
if __name__ == "__main__":
    ...
    for i in range (1):
        ...
        for iter in range (number_of_iteration):
            ...
            # print('x_new = ', x_new)
            # x_new, kappa = gradient(X_new, x, kappa, sigma=0.95, eta=0.1, iter=iter) 
